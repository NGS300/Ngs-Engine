package engine.internal.audio.waveform;
...
class WaveformDataParser{
  ...
  public static function interpretFlxSound(sound:flixel.sound.FlxSound):Null<WaveformData>{
    ...
    @:privateAccess // Method 1. This only works if the sound has been played before.
    /*var soundBuffer:Null<lime.media.AudioBuffer> = sound?._channel?.__source?.buffer;
    if (soundBuffer == null){ // Method 2. This works if the sound has not been played before.
      @:privateAccess
  ...
  public static function interpretAudioBuffer(soundBuffer:lime.media.AudioBuffer):Null<WaveformData>{
    ...
    for (pointIndex in 0...outputPointCount){ // minChannel1, maxChannel1, minChannel2, maxChannel2, ...
      ...
      for (value in values) // We now have the min and max values for the range.
