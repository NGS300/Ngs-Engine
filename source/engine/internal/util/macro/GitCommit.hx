...
class GitCommit{
  ...
  */
  public static macro function getGitCommitHash():haxe.macro.Expr.ExprOf<String>{
    #if !display
    var pos = haxe.macro.Context.currentPos(); // Get the current line number.
    ...
    var commitHash:String = process.stdout.readLine(); // read the output of the process
  ...
  */
  public static macro function getGitBranch():haxe.macro.Expr.ExprOf<String>{
    #if !display // Get the current line number.
  ...
  */
  public static macro function getGitHasLocalChanges():haxe.macro.Expr.ExprOf<Bool>{
    ...
    return macro $v{output.length > 0}; // Generates a string expression
    #else // `#if display` is used for code completion. In this case we just assume true.
    return macro $v{true};
    #end
  }
}
