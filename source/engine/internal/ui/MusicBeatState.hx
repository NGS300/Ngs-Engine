...
class MusicBeatState extends FlxTransitionableState /*implements IEventHandler*/{
  ...
  function handleFunctionControls():Void{
    //if (FlxG.keys.justPressed.F4) FlxG.switchState(() -> new MainMenuState()); // Emergency exit button.
    if (FlxG.keys.justPressed.F5) debug_refreshModules(); // This can now be used in EVERY STATE YAY!
  }
  ...
  function debug_refreshModules(){
    ...
    this.destroy();
    FlxG.resetState(); // Create a new instance of the current state, so old data is cleared.
  }
